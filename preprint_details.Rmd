...
getLinks <- function(doi) {
  ...
  outlinks <- list() #the list where i want the output to be saved in

  for(i in links){if(substring(i, 1, 1)!='#'){outlinks[[length(outlinks)+1]]<- c(i)}}
...
getLinksSafely <- safely(getLinks)
...
links <- map(head(biorxiv_preprints$doi,5), ~ getLinksSafely(.))
